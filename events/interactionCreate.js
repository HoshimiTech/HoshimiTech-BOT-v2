				...
				if (interaction?.customId.startsWith('pomodoro_settings_edit')) {
					...
					const labels = {
						...
						voiceNotificationMessageWorkTime: {
							...
							value: pomodoroSettings.voiceNotification.message.workTime,
						},
						voiceNotificationMessageBreakTime: {
							label: '休憩時間のボイス通知メッセージ',
							value: pomodoroSettings.voiceNotification.message.breakTime,
						},
										...
										new LabelBuilder()
											...
											.setDescription(
												editType === 'voiceNotificationVolume'
													? `単位を付けずに0から100までの整数で入力してください。現在は ${currentValue}%に設定されています。`
													: editType === 'timesUntilLongBreak'
														? `単位を付けずに0以上の整数で入力してください。現在は ${currentValue}回に設定されています。`
