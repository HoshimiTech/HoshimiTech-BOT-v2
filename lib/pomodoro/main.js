...
async function calculateStatusPercentage(pomodoroState) {
...
async function generatePomodoroStatusEmojis(percentage) {
	const doneEmoji = '🟩';
	const yetEmoji = '⬜';
...
async function sendPomodoroStatus(interaction, pomodoroState) {
	...
	const statusPercentage = await calculateStatusPercentage(pomodoroState);
	const statusEmojis = await generatePomodoroStatusEmojis(statusPercentage);
