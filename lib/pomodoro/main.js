...
async function start(client, interaction, options = {}) {
	...
	if (pomodoroState.config.options.voiceNotificationMessage === undefined) {
	...
	try {
		...
		if (
			pomodoroStateConfig?.voiceNotification === undefined ||
			pomodoroStateConfig?.voiceNotification === null
		) {
		...
		if (!pomodoroStateConfig?.voiceNotificationMessage?.workTime) {
			pomodoroStateConfig.voiceNotificationMessage.workTime =
				await pomodoroVoiceUtils.getVoiceNotificationMessage(
		...
		if (
			pomodoroStateConfig?.voiceNotificationSpeakerId === undefined ||
			pomodoroStateConfig?.voiceNotificationSpeakerId === null
		) {
			pomodoroStateConfig.voiceNotificationSpeakerId =
...
async function stop(client, interaction) {
	...
	pomodoroState.paused = false;
	pomodoroState.part.pausedRemainingSeconds = null;
	await timerController.stop(pomodoroState);
	await pomodoroStateUtils.clearPomodoroState(client, guildId);
