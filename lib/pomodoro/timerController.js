...
function startInterval(client, interaction, pomodoroState) {
	...
	pomodoroState.timer = setInterval(async () => {
			...
			try {
				...
				if (channel && channel.members) {
					...
					if (nonBotMembers.size === 0) {
						pomodoroState.vcEmptyElapsedMs += interval;
						if (pomodoroState.vcEmptyElapsedMs >= disconnectedLimit) {
							pomodoroState.part.type = 'cancelled';
							stop(pomodoroState, 'cancelled');
							return;
						}
						// 1時間未満なら一度だけ通知
						if (pomodoroState.vcEmptyElapsedMs === interval) {
							await interaction.channel.send(
								'ℹ️ VCに誰もいなくなりました。1時間以内に誰かが戻らない場合、タイマーは自動で中止されます。',
							);
						}
					} else {
						// 誰か戻ってきたらリセット
						pomodoroState.vcEmptyElapsedMs = 0;
					}
				}
			} catch (err) {
				// チャンネル取得失敗は無視
		...
		const statusPercentage =
			await pomodoroCalculatorUtils.calculateStatusPercentage(pomodoroState);
		// サーバーへの導入数次第で、絵文字の割合を計算
		const statusEmojis =
			await pomodoroMessageUtils.generatePomodoroStatusEmojis(statusPercentage);
		...
		if (pomodoroState.running && shouldUpdate && totalAutoUpdateCount < 10) {
			// ステータスを更新する
			try {
				await pomodoroMessageUtils.sendPomodoroStatus(
					interaction,
					pomodoroState,
				);
			} catch (err) {
