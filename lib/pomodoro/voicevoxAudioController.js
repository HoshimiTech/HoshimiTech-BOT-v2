		...
		for (const speakerId of speakerDirs) {
			...
			audioFiles.forEach((file) => {
				if (file.endsWith('.wav')) {
					if (file === 'workTime.wav') {
						voiceList[guildId][speakerId].audio.workTime = path.join(
							speakerDirPath,
							'workTime.wav',
						);
					} else if (file === 'breakTime.wav') {
						voiceList[guildId][speakerId].audio.breakTime = path.join(
							speakerDirPath,
							'breakTime.wav',
						);
					} else if (file === 'longBreakTime.wav') {
						voiceList[guildId][speakerId].audio.longBreakTime = path.join(
							speakerDirPath,
							'longBreakTime.wav',
						);
					} else if (file === 'stopPomodoro.wav') {
						voiceList[guildId][speakerId].audio.stopPomodoro = path.join(
							speakerDirPath,
							'stopPomodoro.wav',
						);
					}
