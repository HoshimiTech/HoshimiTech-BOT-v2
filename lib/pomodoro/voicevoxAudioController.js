 ...
 */
async function create(audioType, message, speakerId, { guildId = null } = {}) {
	let audioDir;
	if (!guildId) {
		// デフォルト文言を使用する場合
...
async function getAudioFilePath(
	...
	if (isFileNotExist || isServerDataNewer || isMessageDifferent) {
		...
		await create(audioType, message, speakerId, { guildId });
	}
...
async function getSpeakerInfo(speakerId) {
	...
	for (const speaker of speakers) {
		for (const style of speaker.styles) {
			if (!style.type.includes('talk')) return; // talkタイプ以外は無視
			speakerList.push({
				id: style.id,
...
module.exports = { getAudioFilePath, getSpeakerInfo };
