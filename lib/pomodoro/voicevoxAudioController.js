...
const DEFAULT_TEXTS = {
	...
	startLongBreaking:
		'長めの休憩時間が始まります。しっかりとリフレッシュしてください。',
 ...
 * @param {string} text - 音声に使用するテキスト（未指定の場合はデフォルトの文言が使用される）
 * @param {string} guildId - ギルドID（未指定の場合はcommonディレクトリに保存される）
 * @param {string} speakerId - スピーカーのID（未指定の場合は3(ずんだもん)が使用される）
 ...
 */
async function get(audioType, speakerId = 3, { guildId = null } = {}) {
	// ファイルパスの定義
	let audioFilePath;
	if (guildId) {
...
module.exports = { create, get, sync };
